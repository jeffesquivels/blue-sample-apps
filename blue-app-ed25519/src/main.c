...
// Buffer for the message to be signed
static unsigned char message[MAX_MESSAGE_SIZE];
...
io_seproxyhal_touch_approve(const bagl_element_t *e) {
    ...
    if (MAX_MESSAGE_SIZE < (msgSize + G_io_apdu_buffer[4])) {
        THROW(0x6a80);
    }

    // Add more content to the message
    os_memmove(message + msgSize, G_io_apdu_buffer + 5, G_io_apdu_buffer[4]);
    msgSize += G_io_apdu_buffer[4];

    if (G_io_apdu_buffer[2] == P1_LAST) {
        // Message is finalized, send back the signature
        unsigned char privateKeyData[32];
        cx_ecfp_private_key_t privateKey;

        // Key derivation follows SLIP-0010
        os_perso_derive_node_bip32(CX_CURVE_Ed25519, path, 5, privateKeyData, NULL);
        cx_ecfp_init_private_key(CX_CURVE_Ed25519, privateKeyData, 32, &privateKey);
        os_memset(privateKeyData, 0, sizeof(privateKeyData));

        // Very important: contrary to what the documentation states,
        // cx_eddsa_sign() takes the message to be signed directly, not a
        // a previously hashed version.
#if CX_APILEVEL >= 8
        tx = cx_eddsa_sign(&privateKey, CX_LAST, CX_SHA512, message, msgSize,
                           NULL, 0, G_io_apdu_buffer, NULL);
#else
        tx = cx_eddsa_sign(&privateKey, NULL, CX_LAST, CX_SHA512, message,
                           msgSize, G_io_apdu_buffer);
#endif
        ...
        BEGIN_TRY {
            TRY {
                ...
                case INS_GET_PUBLIC_KEY: {
                    ...
                    unsigned char privateKeyData[32];
                    cx_ecfp_public_key_t rawPublicKey;
                    cx_ecfp_private_key_t privateKey;

                    // Key derivation follows SLIP-0010
                    os_perso_derive_node_bip32(CX_CURVE_Ed25519, path, 5,
                                               privateKeyData, NULL);
                    cx_ecfp_init_private_key(CX_CURVE_Ed25519, privateKeyData,
                                             32, &privateKey);
                    os_memset(privateKeyData, 0, sizeof(privateKeyData));
                    cx_ecfp_generate_pair(CX_CURVE_Ed25519, &rawPublicKey,
                                          &privateKey, 1);
                    os_memset(&privateKey, 0, sizeof(privateKey));
                    // copy public key little endian to big endian
                    for (int i = 0; i < 32; i++) {
                        publicKey[i] = rawPublicKey.W[64 - i];
                    }
                    if ((rawPublicKey.W[32] & 1) != 0) {
                        publicKey[31] |= 0x80;
                    }
...
__attribute__((section(".boot"))) int main(void) {
    ...
    msgSize = 0;
    os_memset(message, 0, MAX_MESSAGE_SIZE);

    // Key derivation follows SLIP-0010
    // Very important: ed25519 only supports hardened paths, see
